...
def GET_FRUIT_DATA(user_preferences):
    ...
    def recommend_fruits(user_preferences, fruit_data, num_recommendations=10):
        ...
        liked_fruits = [find_closest_fruit(word, fruit_names) for word in user_preferences.get('likes', [])]
        allergic_fruits = [find_closest_fruit(word, fruit_names) for word in user_preferences.get('allergies', [])]
        ...
        additional_recommendations = [fruit for fruit in fruit_names if fruit not in recommended_fruits and fruit not in allergic_fruits][:remaining_recommendations]
